...
- **Go:** из репозитория дистрибутива
...
- **Git** — из репозитория дистрибутива
...
### **Интеграция в legacy-сборку (build-iso.sh)**

Фаза 2 автоматически интегрируется в сборку ISO через legacy-сборку (`scripts/legacy/build-iso.sh`):
...
3. **Автоматическая установка** — запускается после установки KDE Plasma и до настройки autologin
