...
2. **Установка Core OS и Plasma в chroot:**
   - Установка базовых CLI-утилит (см. `scripts/base/base-packages.sh` и итоговый список пакетов для alpha).
   - Установка KDE Plasma и необходимого набора приложений.
   - Применение cleanup-скриптов (`scripts/base/cleanup.sh`) и базового брендинга.
3. **Подготовка SquashFS и структуры ISO:**
   - `mksquashfs "$CHROOT_DIR" "$IMAGE_DIR"/filesystem.squashfs`.
   - Копирование ядра, initrd, конфигурации GRUB в `$IMAGE_DIR`.
4. **Создание ISO:**
   - `grub-mkrescue -o "$ISO_OUTPUT" "$IMAGE_DIR"`.

На этапе alpha CI будет выполнять только dry-run, а полноценная сборка может запускаться вручную на более мощной машине.
