...
show_main_menu() {
    print_header
    print_section "📦 Выберите компоненты для установки:"
    echo ""

    print_option "1" "Терминал и оболочка (Kitty, Zsh, Starship, CLI-утилиты)" "$(get_status terminal)"
    print_option "2" "Языки программирования (Python, Node.js, Rust, Go, Java)" "$(get_status langs)"
    print_option "3" "Редакторы и IDE (VSCodium, Neovim, Zed)" "$(get_status editors)"
    print_option "4" "Инструменты разработчика (Git, Docker, lazygit)" "$(get_status devtools)"
    print_option "5" "AI-стек (Ollama, Open WebUI, ai-chat, ComfyUI)" "$(get_status ai)"
    print_option "6" "Драйверы NVIDIA" "$(get_status nvidia)"
    echo ""
    print_option "A" "Установить ВСЁ (Full версия)" ""
    print_option "R" "Перепроверить установленные компоненты" ""
    print_option "0" "Выход" ""
    echo ""
    echo -e "${YELLOW}Введите номер компонента (или несколько через пробел):${NC}"
}
...
install_ai() {
    ...
    if [[ ! -f /usr/local/bin/ai-chat ]]; then
        ...
        # Простой скрипт для чата с Ollama
